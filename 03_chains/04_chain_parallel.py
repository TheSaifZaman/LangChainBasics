"""
Parallel branching with RunnableParallel.

What this shows
- How to fan out work into parallel branches, each being its own chain.
- How to combine results from branches into a single output.
- Reuses the same model for multiple sub-chains.

Run
    python 03_chains/04_chain_parallel.py
"""
...
# Simplify branches with LCEL (each branch is a mini chain)
plot_branch_chain = (
...
# 1. Prompt for a summary
# 2. Parse the summary text
# 3. Run two branches in parallel (plot and character analysis)
# 4. Combine their outputs into one verdict
chain = (
    summary_template
    | model
    | StrOutputParser()
